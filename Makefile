...
# These definitions are from config.sh (via /usr/local/lib/perl5/i486-linux/5.002/Config.pm)
...
SITEARCHEXP = /usr/local/lib/perl5/site_perl/i486-linux
SO = so
...
INSTALLARCHLIB = $(PREFIX)/lib/perl5/i486-linux/5.002
INSTALLSITELIB = $(PREFIX)/lib/perl5/site_perl
INSTALLSITEARCH = $(PREFIX)/lib/perl5/site_perl/i486-linux
INSTALLBIN = $(PREFIX)/bin
PERL_LIB = /usr/local/lib/perl5
PERL_ARCHLIB = /usr/local/lib/perl5/i486-linux/5.002
SITELIBEXP = /usr/local/lib/perl5/site_perl
SITEARCHEXP = /usr/local/lib/perl5/site_perl/i486-linux
LIBPERL_A = libperl.a
...
PERLMAINCC = $(CC)
PERL_INC = /usr/local/lib/perl5/i486-linux/5.002/CORE
...
$(INST_AUTODIR)/.exists :: /usr/local/lib/perl5/i486-linux/5.002/CORE/perl.h
	@$(MKPATH) $(INST_AUTODIR)
	@$(EQUALIZE_TIMESTAMP) $(PERL) $(INST_AUTODIR)/.exists
	-@$(CHMOD) 755 $(INST_AUTODIR)

$(INST_LIBDIR)/.exists :: /usr/local/lib/perl5/i486-linux/5.002/CORE/perl.h
	@$(MKPATH) $(INST_LIBDIR)
	@$(EQUALIZE_TIMESTAMP) $(PERL) $(INST_LIBDIR)/.exists
	-@$(CHMOD) 755 $(INST_LIBDIR)

$(INST_ARCHAUTODIR)/.exists :: /usr/local/lib/perl5/i486-linux/5.002/CORE/perl.h
...
$(INST_MAN3DIR)/.exists :: /usr/local/lib/perl5/i486-linux/5.002/CORE/perl.h
	@$(MKPATH) $(INST_MAN3DIR)
...
# installpm: CGI.pm => $(INST_LIBDIR)/CGI.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI.pm: CGI.pm Makefile $(INST_LIBDIR)/.exists $(INST_ARCHAUTODIR)/.exists
	@rm -f $@
	$(UMASK_NULL) && cp CGI.pm $@
	@$(AUTOSPLITFILE) $@ $(INST_LIB)/auto


# installpm: CGI/Carp.pm => $(INST_LIBDIR)/CGI/Carp.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/CGI/Carp.pm: CGI/Carp.pm Makefile $(INST_LIBDIR)/CGI/.exists $(INST_ARCHAUTODIR)/.exists
	@rm -f $@
	$(UMASK_NULL) && cp CGI/Carp.pm $@
	@$(AUTOSPLITFILE) $@ $(INST_LIB)/auto

$(INST_LIBDIR)/CGI/.exists :: /usr/local/lib/perl5/i486-linux/5.002/CORE/perl.h
	@$(MKPATH) $(INST_LIBDIR)/CGI
	@$(EQUALIZE_TIMESTAMP) $(PERL) $(INST_LIBDIR)/CGI/.exists
...
clean ::
	-rm -rf ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all perlmain.c mon.out core so_locations *~ */*~ */*/*~ *$(OBJ_EXT) *$(LIB_EXT) perl.exe $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def $(BASEEXT).exp
